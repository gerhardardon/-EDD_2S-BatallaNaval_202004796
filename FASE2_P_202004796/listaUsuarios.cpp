...
public:
    ...
    nodo(int id1, string nombre, string contra, int monedas, int edad) { //Constructor
        id = id1;
        name = nombre;
        password = contra;
        age = edad;
...
public:
    ...
    void insert(int id1, string nombre, string contra, int monedas, int edad) {
        nodo* nuevo = new nodo(id1, nombre, contra, monedas, edad);
        if (primero == NULL) {
            primero = nuevo;
        }
        else {
   ...
   /* void agregarAvl(string name, nodoavl* arbol) {
        nodo* aux = primero;
        while (aux != NULL) {
            if (aux->name == name) {
                aux->arbolAvl = arbol;
    ...
    void imprimir() {
        nodo* aux = primero;
        while (aux != NULL) {
            cout << "ID: " << aux->id << "NOMBRE: " << aux->name << "  CONTRASEÑA: " << aux->password << "  MONEDAS: " << aux->coins << "  EDAD: " << aux->age << endl;
            while (aux->siguiente == primero) {
                return;
            }
            aux = aux->siguiente;
