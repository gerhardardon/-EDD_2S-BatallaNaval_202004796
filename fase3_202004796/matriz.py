...
##imprimir la matriz Nota: este metodo tambien esta imprimiendo las cabeceras
    ...
    def realizarAtaque(self,x,y,dimension):    
        if self.buscarContenido(x,y) != "white":
            self.insertarNodo(x,y,"x")
            print("acertado")
        
            tkinter.messagebox.showinfo(message="Le has dado!!!", title="ACERTADO")
            self.confirmarGanar()
            
        else:
            if x>dimension or y>dimension:
                tkinter.messagebox.showinfo(message="ingresa una coordenada correcta :/", title="OUT OF BOUNDS")
            else:
                self.insertarNodo(x,y,"o")
                print("fallado")
                tkinter.messagebox.showinfo(message="Has fallado!!!", title="FALLADO")
    ...
    def confirmarGanar(self):
        faltantes=0
        aux=self.root
        while aux is not None:
            aux2=aux
            while aux2 is not None:
                if aux2.dato=="P" or aux2.dato=="s" or aux2.dato=="d" or aux2.dato=="b":
                    faltantes+=1
                aux2=aux2.siguiente
            aux=aux.abajo

        if faltantes==0:
            tkinter.messagebox.showinfo(message="Ha hundido todos los barcos!!!", title="GANADOR")
            return True
        else:
            return False
